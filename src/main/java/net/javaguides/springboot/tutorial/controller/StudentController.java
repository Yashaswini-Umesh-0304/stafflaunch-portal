...
public class StudentController {
    ...
    public StudentController(StudentRepository repository, TechIssueRepository techIssueRepository, PasswordEncoder encoder) {
    ...
    @GetMapping("/signup")
    public String signup(Student student) { return "add-student"; }

    @PostMapping("/add-user")
    public String register(@Valid Student student, BindingResult result) {
        if (result.hasErrors()) return "add-student";
        Optional<Student> existingUser = repository.findByEmail(student.getEmail());
        if (existingUser.isPresent()) {
            Student existing = existingUser.get();
            if (existing.isEnabled() && (existing.getPassword() == null || existing.getPassword().isEmpty())) {
                existing.setUsername(student.getUsername());
                existing.setPassword(encoder.encode(student.getPassword()));
    ...
    @PostMapping("/students/add-manual")
    public String adminAddManual(Student student) {
        student.setRole("ROLE_USER");
        student.setEnabled(true);
        student.setAssetAcknowledged(true);
        repository.save(student);
    ...
    public String dashboard(Model model) {
        ...
        Student user = repository.findByEmailOrUsername(currentUsername).orElseThrow();
        if (user.getRole().equals("ROLE_ADMIN")) {
            var all = repository.findAll();
            model.addAttribute("pendingCount", all.stream().filter(s -> !s.isEnabled()).count());
            model.addAttribute("activeCount", all.stream().filter(Student::isEnabled).count());
        ...
        model.addAttribute("myIssues", techIssueRepository.findByStudentIdOrderByReportDateDesc(user.getId()));
        return "employee-dashboard";
    }

    @PostMapping("/students/update/{id}")
    public String update(@PathVariable("id") long id, @ModelAttribute("student") Student student) {
        ...
        if (isAdmin) return "redirect:/students/list";
        return "redirect:/login?logout"; 
    }

    @GetMapping("/students/list")
    public String list(Model model) {
        model.addAttribute("students", repository.findAll().stream().filter(Student::isEnabled).collect(Collectors.toList()));
        return "index";
    }

    @PostMapping("/students/acknowledge-asset")
    public String acknowledge() {
        String currentUsername = SecurityContextHolder.getContext().getAuthentication().getName();
        Student user = repository.findByEmailOrUsername(currentUsername).orElseThrow();
        user.setAssetAcknowledged(true);
        repository.save(user);
        return "redirect:/students/dashboard?assetSuccess";
    }

    @PostMapping("/students/claim-asset")
    public String claimAsset(@RequestParam("assets") String assets) {
        ...
        user.setAssetAcknowledged(true); 
        repository.save(user);
        return "redirect:/students/dashboard?claimSuccess";
    }

    @GetMapping("/students/report-issue")
    ...
    @PostMapping("/students/submit-issue")
    public String submitIssue(@RequestParam("asset") String asset, 
        ...
        return "redirect:/students/dashboard?issueReported";
    }

    @PostMapping("/students/cancel-issue/{id}")
    public String cancelIssue(@PathVariable("id") long id) {
        ...
        techIssueRepository.save(issue);
        return "redirect:/students/dashboard?issueCancelled";
    }
    ...
    @GetMapping("/students/approvals")
    public String showApprovals(Model model) {
        model.addAttribute("pendingUsers", repository.findAll().stream().filter(s -> !s.isEnabled()).collect(Collectors.toList()));
        return "approvals";
    }

    @GetMapping("/students/approve/{id}")
    public String approve(@PathVariable("id") long id) {
        Student s = repository.findById(id).orElseThrow();
        s.setEnabled(true);
        repository.save(s);
        return "redirect:/students/approvals";
    }
    ...
    @PostMapping("/students/resources/{slug}/mark-read")
    public String markArticleRead(@PathVariable String slug) {
        ...
        return "redirect:/students/resources/" + slug;
    }
}